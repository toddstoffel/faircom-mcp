...
> Current maintainer release target: v0.1.10. Update the package metadata and release docs together before tagging a new release.
...
```bash
devtools/publish_release.sh v0.1.10
```
...
devtools/publish_release.sh v0.1.10 --no-wait

# Allow local uncommitted changes (advanced; not recommended)
devtools/publish_release.sh v0.1.10 --allow-dirty
```
